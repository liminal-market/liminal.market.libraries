...
export default class FormValidator {
  ...
  public validateRequiredFields() {
    let inputs = document.querySelectorAll(
      this.selector + " input[required], " + this.selector + " select[required]"
    );
    for (let i = 0; i < inputs.length; i++) {
      ...
      if (this.isMissingInputFromUser(input)) {
        let obj = {
          ...
          inputName: input.id,
          labelText: document.querySelector("label[for=" + input.id + "]")!
            .innerHTML,
      ...
      if (input.pattern) {
        ...
        if (matches == null) {
          let obj = {
            message:
              "This is not valid date format, please use YYYY-MM-DD (year-month-date)",
            inputName: input.id,
            labelText: document.querySelector("label[for=" + input.id + "]")!
              .innerHTML,
            pattern: input.pattern,
  ...
  private isMissingInputFromUser(input: HTMLInputElement) {
    return (
      ...
      StringHelper.isNullOrEmpty(input.value)
    );
  }
  ...
  private handle(obj: {
    ...
    let inputError = input.parentElement!.querySelector(
      ".input_error"
    ) as HTMLElement;
    if (inputError) {
    ...
    input.insertAdjacentHTML(
      "beforebegin",
      '<div class="input_error" style="display: block" id="input_error_' +
        obj.inputName +
        '">' +
        obj.message +
        "</div>"
