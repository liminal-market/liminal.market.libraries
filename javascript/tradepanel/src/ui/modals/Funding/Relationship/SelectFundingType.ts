...
export default class SelectFundingType {
  ...
  private bindEvent() {
    let next_bank_information = document.getElementById(
      "next_bank_information"
    );
    next_bank_information?.addEventListener("click", (evt) => {
      this.aUsdFund.hideError("selectFundingTypeError");

      let ach_transfer = document.getElementById(
        "ach_transfer"
      ) as HTMLInputElement;
      if (ach_transfer?.checked) {
        this.aUsdFund.achRelationship.show();
      }
      let wire_transfer = document.getElementById(
        "wire_transfer"
      ) as HTMLInputElement;
      ...
      this.aUsdFund.showError(
        "selectFundingTypeError",
