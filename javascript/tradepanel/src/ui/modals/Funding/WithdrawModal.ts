...
export default class WithdrawModal {
  ...
  public async show() {
    ...
    if (!bankRelationships) {
      modal.showModal(
        ...
        "You haven't setup bank connection. You cannot withdraw from without bank connection"
      );
      return;
    }

    let transfers = await this.userService.getLatestTransfers(
      TransferDirectionEnum.Outgoing
    );
    let transfersHtml = await this.transfersList.render(
      TransferDirectionEnum.Outgoing,
      transfers
    );
    ...
    if (this.currentBalance.eq(0)) {
      let tmp = WidgetGlobals.HandlebarsInstance.compile(
        "Your current balance is $0. There is nothing to withdraw. {{{transfers}}}"
      );
      modal.showModal(
        "Withdraw information",
        tmp({ transfers: transfersHtml })
      );
      return;
    }
    let withdrawTemplate =
      WidgetGlobals.HandlebarsInstance.compile(WithdrawModalHtml);
  ...
  private bindEvents() {
    ...
    confirmWithdraw?.addEventListener("click", async (evt) => {
      ...
      if (StringHelper.isNullOrEmpty(amount.value)) {
        ...
        let withdrawAmountError = document.getElementById(
          "withdrawAmountError"
        );
  ...
  private showWithdrawConfirmation() {
    ...
    feePercentage!.innerText = roundNumber(
      (this.transferCost / parseFloat(amount.value)) * 100
    ).toString();

    let confirmWithdrawButton = document.getElementById(
    ...
    confirmWithdrawButton?.addEventListener("click", async (evt) => {
      ...
      await this.userService
        ...
        .then(async (response) => {
          ...
          withdrawConfirm!.innerHTML = await this.transfersList.render(
            TransferDirectionEnum.Outgoing,
            transfers
          );
          this.transfersList.bindEvents();
