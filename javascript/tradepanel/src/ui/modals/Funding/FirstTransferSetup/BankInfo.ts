...
export default class BankInfo extends FirstTransferSetupBase {
  ...
  public show() {
    let bankInfoTemplate =
      WidgetGlobals.HandlebarsInstance.compile(BankInfoHtml);
  ...
  public bindEvents() {
    ...
    next?.addEventListener("click", (evt) => {
      ...
      if (this.bankRelationship.transfer_type == "wire") {
        let wireTransferMoney = new WireTransferMoney(
          this.aUsdFund,
          this.bankRelationship
        );
        wireTransferMoney.show();
      } else {
        let ach = new ACHTransferAccountNumber(
          this.aUsdFund,
          this.bankRelationship
        );
        ach.show();
