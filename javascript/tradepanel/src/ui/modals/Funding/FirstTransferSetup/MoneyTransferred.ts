...
export default class MoneyTransferred extends FirstTransferSetupPage {
  ...
  show(): void {
    let template =
      WidgetGlobals.HandlebarsInstance.compile(MoneyTransferredHtml);
  ...
  bindEvents(): void {
    ...
    notifyTransfer?.addEventListener("click", async (evt) => {
      ...
      if (
        StringHelper.isNullOrEmpty(amount.value) ||
        parseFloat(amount.value) < 10
      ) {
        this.aUsdFund.showError(
          "amountError",
          "Amount cannot be empty or below $10"
        );
      }
      ...
      await userService
        ...
        .then(() => {
          this.aUsdFund.transferNotified.show(
