  ...
  public show() {
    ...
    setNetworkLinks.forEach((setNetworkLink) => {
      setNetworkLink.addEventListener("click", async (evt) => {
        ...
        this.selectedNetwork = NetworkInfo.getNetworkInfoByChainId(
          parseInt(dataset.chainid!)
        );
        ...
        let successAddingNetwork = await walletHelper
          ...
          .catch((error: GeneralError) => {
            let jsSwitchNetworkNotWorking = document.getElementById(
              "jsSwitchNetworkNotWorking"
            );
            ...
            let switchNetworkInfo =
              document.getElementById("switchNetworkInfo")!;
            switchNetworkInfo.classList.add("d-none");

            (
              document.getElementById("switchChainId") as HTMLInputElement
            ).value =
              dataset.chainid! +
              " or it might be: " +
              "0x" +
            ...
            (
              document.getElementById("switchChainName")! as HTMLInputElement
            ).value = this.selectedNetwork!.Name;
            (
              document.getElementById("switchCurrencyName")! as HTMLInputElement
            ).value = this.selectedNetwork!.NativeCurrencyName;
            (
              document.getElementById("switchSymbol")! as HTMLInputElement
            ).value = this.selectedNetwork!.NativeSymbol;
            (
              document.getElementById("switchDecimal")! as HTMLInputElement
            ).value = this.selectedNetwork!.NativeDecimal.toString();
            (
              document.getElementById("switchRpcUrl")! as HTMLInputElement
            ).value = this.selectedNetwork!.RpcUrl;
          });
