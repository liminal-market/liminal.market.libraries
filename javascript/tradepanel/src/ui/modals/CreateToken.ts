...
export default class CreateToken {
  ...
  public show(symbol: string) {
    ...
    let template = WidgetGlobals.HandlebarsInstance.compile(CreateTokenHtml);

    modal.showModal(
      "Token " + symbol + " doesn't exist",
    ...
    btn.addEventListener("click", async () => {
      ...
      if (address === AddressZero) {
        ...
        if (
          result instanceof BlockchainError &&
          result.code == BlockchainError.UserCancelled
        ) {
