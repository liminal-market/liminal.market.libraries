...
export default class KycTrustedContact extends KycBase {
  ...
  private validate() {
    ...
    let trusted_first_name = document.getElementById(
      "trusted_first_name"
    ) as HTMLInputElement;
    if (
      trusted_first_name &&
      ...
      let trusted_email = document.getElementById(
        "trusted_email"
      ) as HTMLInputElement;
      let trusted_phone = document.getElementById(
        "trusted_phone"
      ) as HTMLInputElement;
      let trusted_street_address = document.getElementById(
        "trusted_street_address"
      ) as HTMLInputElement;
      let trusted_city = document.getElementById(
        "trusted_city"
      ) as HTMLInputElement;
      let trusted_postal_code = document.getElementById(
        "trusted_postal_code"
      ) as HTMLInputElement;
      let trusted_country = document.getElementById(
        "trusted_country"
      ) as HTMLInputElement;

      if (
        StringHelper.isNullOrEmpty(trusted_email.value) &&
        StringHelper.isNullOrEmpty(trusted_phone.value) &&
        StringHelper.isNullOrEmpty(trusted_street_address.value)
      ) {
        this.setMissingInfo(
          "trusted_contact_missing_info",
          "You need to fill in either email, phone or address"
        );
        return;
      }
      ...
      if (
        ...
        this.setMissingInfo(
          "trusted_contact_missing_info",
          "Email is not valid. It must contain @ sign"
        );
        return;
      }
      ...
      if (
        ...
        this.setMissingInfo(
          "trusted_contact_missing_info",
          "You need fill inn into City, Postal code and Country when filling the street address"
        );
        return;
      }
    }
