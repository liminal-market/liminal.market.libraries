...
export default class KycIdentity extends KycBase {
  ...
  public bindEvents() {
    this.bind("#citizen_yes", "click", (evt) => {
      ...
      if (input.checked) {
        let taxIdType = document.getElementById(
          "tax_id_type"
        ) as HTMLSelectElement;
  ...
  private validateInputs() {
    ...
    let citizen_yes = document.getElementById(
      "citizen_yes"
    ) as HTMLInputElement;
    ...
    let greenCardOption = document.getElementById(
      "citizen_no_type_options_1"
    ) as HTMLInputElement;
    let visaOption = document.getElementById(
      "citizen_no_type_options_2"
    ) as HTMLInputElement;
    if (citizen_no.checked && !greenCardOption.checked && !visaOption.checked) {
      let obj = {
        message:
          'You must select either option, "Green Card / Permanent Resident" or "Visa"',
